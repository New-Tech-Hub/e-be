...
USING (
  ...
  public.log_security_event(
    ...
    'profiles', 
    user_id,
    jsonb_build_object('viewed_by', auth.uid())
  ) IS NOT NULL
);
...
WITH CHECK (
  ...
  public.log_security_event(
    ...
    'profiles',
    user_id,
    jsonb_build_object('updated_by', auth.uid(), 'new_data', to_jsonb(NEW))
  ) IS NOT NULL
);
...
-- Prevent role escalation with enhanced logging
DROP TRIGGER IF EXISTS prevent_role_escalation ON public.profiles;
CREATE TRIGGER prevent_role_escalation_with_logging
